...
API_NAVITIA = "https://api.sncf.com/v1/coverage/sncf/journeys?key=6cf28a3a-59c3-4c82-8cbf-8fa5e64b01da&from=admin:fr:{0}&to=admin:fr:{1}&datetime={2}"
...
def compute_fitness(solution):
    ...
    for index in range(len(solution)):
        ...
        try:
            ...
            print("tuple not found '{}' & '{}'".format(waypoint1, waypoint2))
...
def run_genetic_algorithm(generations=5000, population_size=70):
    """
...
for (waypoint1, waypoint2) in combinations(all_waypoints, 2):
    try:
        ...
        duration =  response["journeys"][0]["duration"]
        co2 = response["journeys"][0]["co2_emission"]["value"]
        for journey in response["journeys"]:
            if duration > journey["duration"]:
                duration = journey["duration"]
            if co2 > journey["co2_emission"]["value"]:
        ...
        waypoint_co2[frozenset([waypoint1, waypoint2])] = co2
...
for i, row in waypoint_data.iterrows():
    waypoint_co2[frozenset([row.waypoint1, row.waypoint2])] = row.co2_gEC
