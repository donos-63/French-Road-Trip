...
# Import de la fonction permettant de travailler ac des fichiers CSV (writer, reader)
import csv
...
def truncate_roadtrip():
    ...
    """
    # Suppression du rendu visuel du trajet
...
def initialize_schema():
    ...
    with db_connector:
        ...
        # CREATION TABLES
        result = db_connector.execute_query(
            "SELECT to_regclass('french_trip');")
...
def load_prefecture():
    ...
    with db_connector:
        try:
            db_connector.execute_nonquery(sql.SQL_REINIT_PREFECTURE)
            print("Vidage de la table 'prefecture'")
            ...
            with open(CITY_REFERENTIAL_OUT_PATH, mode="r", encoding="utf-8") as file:
                ...
                for row in dic:
                    ...
                    db_connector.execute_nonquery(
                        sql.SQL_INSERT_PREFECTURE, values)
            ...
            print("Problème durant le chargement des préfectures. Rollback effectué")
